...
if __name__ == "__main__":
    if len(sys.argv) <= 2:
        ...
        sys.exit(0)
        # 檢查攻擊階段的繳交物(推測行號資料E)的格式

    dfB = pd.read_csv(sys.argv[1], header=None)
    dfE = pd.read_csv(sys.argv[2], header=None)
    n = dfB.shape[0]

    print(dfE.shape, 'OK' if dfE.shape[0] ==
          100 and dfE.shape[1] == 3 else 'Invalid')
    print('E: int',  'OK' if dfE.dtypes.isin(
        [np.dtype('int32'), np.dtype('int64')]).all() else 'invalid')
